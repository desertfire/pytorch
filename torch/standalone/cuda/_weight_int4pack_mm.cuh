...
// This is a copy-paste of aten/src/ATen/native/cuda/int4mm.cu with
// _weight_int4pack_mm_cuda taking template parameters.
...
T _weight_int4pack_mm_cuda(
  ...
  // TODO: Unify CudaGuard and AOTICudaGuard
  // torch::standalone::AOTICudaGuard g(A.device());
  ...
  TORCH_CHECK(A.dtype() == torch::standalone::ScalarType::BFloat16);
  TORCH_CHECK(A.is_contiguous());
  TORCH_CHECK(A.dim() == 2);

  // B has B_innerKTiles k-tiles in the innermost dimension
  TORCH_CHECK(B.dtype() == torch::standalone::ScalarType::Int);
