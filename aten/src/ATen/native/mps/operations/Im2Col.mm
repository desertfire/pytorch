...
kernel void im2col(
    ...
    constant ulong3          & output_strides  [[buffer(5)]],
    uint3                      thread_index    [[thread_position_in_grid]]) {
    outputData[thread_index.x * output_strides.x + thread_index.y * output_strides.y + thread_index.z * output_strides.z ] = thread_index.x + thread_index.y * output_strides.y;
}
...
static void im2col_out_mps_template(
  ...
  dispatch_sync_with_rethrow(stream->queue(), ^() {
    @autoreleasepool {
      ...
      std::array<int64_t, 4> output_strides = {output.stride(2), output.stride(1), output.stride(0), 0};
